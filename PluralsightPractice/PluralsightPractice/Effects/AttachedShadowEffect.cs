...
{
   public static class ShadowEffectParameters
   {
      ...
      static void OnHasShadowChanged(BindableObject bindable, object oldValue, object newValue)
      {
         ...
         if (hasShadow)
         {
            view.Effects.Add(new AttachedShadowEffect());
         }
         else
         {
            var toRemove = view.Effects.FirstOrDefault(e => e is AttachedShadowEffect);
            if (toRemove != null)
            {
      ...
      class AttachedShadowEffect : RoutingEffect
      {
         public AttachedShadowEffect() : base($"Pluralsight.{nameof(AttachedShadowEffect)}")
         {
         }
      }
   }
